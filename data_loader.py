...
def var_subseq(mother_promoter, origin_promoter):
    ...
    # padding hasta seq_len
    origin_promoter += 'B' * max(0, cfg.seq_len - len(origin_promoter))
    mother_promoter += 'B' * max(0, cfg.seq_len - len(mother_promoter))

    for i in range(cfg.seq_len):
        if origin_promoter[i] != mother_promoter[i]:
            mer = [
                vocab[origin_promoter[i]] if 0 <= k < cfg.seq_len else vocab_size
                for k in range(
                    i - ((cfg.mer - 1) // 2),
                    i + (cfg.mer - ((cfg.mer - 1) // 2))
                )
            ]
    ...
    # 🔹 muestreo si hay demasiadas mutaciones
    if len(vars) > cfg.num_var:
        sel = np.random.choice(len(vars), cfg.num_var, replace=False)
        vars = [vars[i] for i in sel]
        indices = [indices[i] for i in sel]

    # 🔹 padding si hay pocas
    pad_var = np.tile(one_hot(vocab_size, vocab_size), cfg.mer)
    while len(vars) < cfg.num_var:
        vars.append(pad_var)
        indices.append(-1)
